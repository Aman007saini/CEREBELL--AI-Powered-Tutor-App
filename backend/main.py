...
app= FastAPI(
    title="CEREBEL",
    description="API for genrating personalized tutoring content and quizzes",
    version="1.0.0"
)
...
class TutorRequest(BaseModel):
    ...
    learning_style: str= Field("Text-based",description="Preferred learning style (Visual, Text-based, Hands-on)")
    background: str= Field("Unknown", description='Background knowlegde level')
    language: str= Field("English", description='Preferred language')

class QuizRequest(BaseModel):
    subject: str = Field(..., description="Academic subject")
    topic: str= Field(..., description="topic on which quize is based")
...
@app.get("/quiz-html/{subject}/{topic}/{level}/{num_questions}", response_class=HTMLResponse)
async def get_quiz_html(subject: str,topic:str, level: str, num_questions: int = 5):
    """
    Get a formatted HTML quiz page.
    """
    try:
        quiz_result = generate_quiz(subject,topic, level, num_questions, reveal_answer=True)
